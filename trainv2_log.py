        ...
        for phase in ['train', 'valid']:
            ...
            for ii, (index, im_a, im_b, label) in enumerate(dataloader[phase]):
                ...
                if ii % args.log_freq == 0 and phase == "train":
                    niter = epoch * len(dataloader[phase]) + ii
                    writer.add_scalars('Train_val_loss', {'train_loss': loss.data[0]}, niter)
                    print (" | Epoch{}: {}, Loss {:.4f}".format(epoch, ii, loss.data[0]))
                running_loss += loss.data[0]
            epoch_loss = running_loss / (ii+1)
            if phase == 'train':
                nitem = (epoch+1) * (len(dataloader['train']))
                writer.add_scalars('Train_val_loss', {'train_loss': loss.data[0]}, nitem)
            if phase == 'valid':
                nitem = (epoch+1) * (len(dataloader['train']))
                writer.add_scalars('Train_val_loss', {'val_loss': epoch_loss}, nitem)

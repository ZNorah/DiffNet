    ...
    def forward(self, pred, label, seen):
        ...
        for b in range(BS):
            ...
            for truth_iter in torch.arange(len(gtb)):
                ...
                coords_loss_scale = 2 - truth_x.data[0] * truth_y.data[0]
                coords_loss = self.coord_scale * coords_loss_scale * (pred_coords.unsqueeze(0) - truth_coords)
...
def bbox_ious(bboxes1, bboxes2, is_anchor):
    x1, y1, w1, h1 = bboxes1.chunk(4, dim=-1) #把tensor按顺序分成4块
    x2, y2, w2, h2 = bboxes2.chunk(4, dim=-1)
    x11 = x1 - 0.5*w1#预测框左上x
    y11 = y1 - 0.5*h1#预测框左上y
    x12 = x1 + 0.5*w1#预测框右下x
    y12 = y1 + 0.5*h1#预测框右下y
    x21 = x2 - 0.5*w2#(1,1)
    y21 = y2 - 0.5*h2
    x22 = x2 + 0.5*w2
    y22 = y2 + 0.5*h2
    xI1 = torch.max(x11, x21.transpose(1, 0))#所有预测框左上角与gt左上角  最大值
    yI1 = torch.max(y11, y21.transpose(1, 0))
            
    xI2 = torch.min(x12, x22.transpose(1, 0))#所有预测框右下角与gt右下角  最小值
...
def bbox_gious(bboxes1, bboxes2):
    ...
    yI2 = torch.min(y12, y22.transpose(1, 0))
    inter_box_w = torch.clamp((xI2 - xI1), min=0)
    inter_box_h = torch.clamp((yI2 - yI1), min=0)
    inter_area = inner_box_w * inner_box_h

    union = bboxea1_area + bboxes2_area.transpose(1, 0) - inter_area
    iou = torch.clamp(inter_area / union, min=0)

    xE1 = torch.min(x11, x21.transpose(1, 0))
    yE1 = torch.min(y11, y21.transpose(1, 0))
    xE2 = torch.max(x12, x22.transpose(1, 0))
    yE2 = torch.max(y12, y22.transpose(1, 0))
    enclose_w = torch.clamp((xE2 - xE1), min=0)
    enclose_h = torch.clamp((yE2 - yE1), min=0)
